...
- :mod:`siscadro_rw5.parser` and :mod:`siscadro_rw5.models`: the raw,
  format-specific parser and record models, useful for CAD-oriented or
  other non-canonical consumers.
- :mod:`siscadro_rw5.extractor`: :class:`~siscadro_rw5.extractor.
  Rw5Extractor`, the adapter that maps raw records onto
  ``siscadro-survey``'s canonical survey point model.
- This module's :func:`extract_points`, :func:`export_to_xlsx`, and
  ...
  database and workbook mechanics to ``siscadro-survey``, so this package
...
from siscadro_survey import database as survey_database
from siscadro_survey import extractors as survey_extractors
from siscadro_survey.records import ExtractionResult, ImportSummary, XlsxSummary
